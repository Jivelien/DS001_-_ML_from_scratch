...
class linear_regression(object):
    def __init__(self, theta_lenght, learning_rate):
        ...
        #self.hyp_func = self.sin_func
        self.hyp_func = self.polynomial_func 

    def init_theta(self, i: int):
        #return np.random.rand(i)
        return np.zeros(i)

    def polynomial_func(self, value, theta):
        return sum(theta[i] * value**i for i in range(theta.size))
    
    def sin_func(self, value, theta):
        return theta[0] * np.sin(theta[1] * value + theta[2]) + theta[3]
    ...
    def gradient_descent(self, x, y):  # learning rate : alpha
        ...
        for i in range(self.theta.size):
            ...
            new_theta[i] = t_old[i] - self.learning_rate * \
                (new_cost - cost) / 0.000000001
