...
lr = linear_regression(theta_lenght = 2 , learning_rate = 0.1)
...
while True:
    if d.number_of_points >= 2 :
        ...
        ys = rescale(lr.hyp_func(np.arange(0,1,0.001), lr.theta), 1, 0, 0, c.height)
        xs = rescale(np.arange(0,1,0.001), 0, 1, 0, c.width)
        
        for i in range(len(ys)):
            if ys[i] >=0 and ys[i] < c.width:
                cc.add_point(int(xs[i]), int(ys[i]))
        img = cv2.cvtColor(c.canva.astype('uint8'), cv2.COLOR_GRAY2RGB)
        img[:,:,canva.color.RED] = cv2.addWeighted(c.canva ,1, cc.canva,1,0)
    else:
        img = c.canva
